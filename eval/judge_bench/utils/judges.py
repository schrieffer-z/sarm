...
class SARM(Judge):
    def __init__(self, model_name, device="cuda:0"):
        ...
        self.device = device
        sarm_params = parse_sarm_param(model_name)
        self.rm = LlamaSARM.from_pretrained(
            model_name,
    ...
    async def get_judgment(self, question: str, answer_A: str, answer_B: str) -> Dict[str, Any]:
        ...
        def get_last_assistant_masks(input_ids):
            ...
            return torch.tensor(assistant_masks, device=input_ids.device).unsqueeze(0)
        ...
        if self.rm_tokenizer.bos_token!=None:
            conv1_formatted=conv1_formatted.replace(self.rm_tokenizer.bos_token, "")
            conv1_formatted=conv1_formatted.replace(self.rm_tokenizer.bos_token, "")

        conv1_tokenized = self.rm_tokenizer(conv1_formatted, truncation=True, return_tensors='pt').to(self.device)
        conv2_tokenized = self.rm_tokenizer(conv2_formatted, truncation=True, return_tensors='pt').to(self.device)
        
        conv1_tokenized.update(
            {"assistant_masks": get_last_assistant_masks(conv1_tokenized['input_ids'].squeeze())}
        )
        conv2_tokenized.update(
            {"assistant_masks": get_last_assistant_masks(conv2_tokenized['input_ids'].squeeze())}
        )
